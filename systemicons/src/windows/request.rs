...
use winapi::{STRUCT,
        ...
        um::{
            ...
            winnt::{HANDLE, FILE_ATTRIBUTE_DIRECTORY, FILE_ATTRIBUTE_NORMAL},
...
pub fn get_icon(ext: &str, size: i32, dir: bool) -> Result<Vec<u8>, Error> {

    fn get_icon_from_ext(ext: &str, size: i32, dir: bool)->HICON {
        unsafe {
            ...
            for _ in 0..3 {
                ...
                SHGetFileInfoW(p_path.as_ptr(), if dir { FILE_ATTRIBUTE_DIRECTORY } else { FILE_ATTRIBUTE_NORMAL }, &mut file_info, file_info_size,
    ...
    unsafe {
        let mut icon = if ext.to_lowercase().ends_with(".exe") {
            ...
            if icon == ptr::null_mut() {
                if let Some(pos) = ext.find(".exe") {
                    icon = get_icon_from_ext(&ext[pos..], size, dir);
                } else {
            ...
            get_icon_from_ext(ext, size, dir)
        };
        if icon == ptr::null_mut() {
            icon = extract_icon("C:\\Windows\\system32\\SHELL32.dll", size);
        }
        ...
        let mut png_bytes: io::Cursor<Vec<u8>> = io::Cursor::new(Vec::new());
